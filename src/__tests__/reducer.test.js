...
const initialState = {
    rates: [
        ...
        { "currency": "dolar kanadyjski", "code": "CAD", "bid": 2.8842, "ask": 2.9424, favourite: true },
        ...
        { "currency": "korona czeska", "code": "CZK", "bid": 0.1666, "ask": 0.17, favourite: true },
...
describe('Reducers', () => {
    ...
    describe('Clear all favourites', () => {
        it('should set all favourite prop to false', () => {
            const action = clearAllFavourites();
            const state = reducer(initialState, action);
            const areAllNotFavourite = state.rates.every(item => item.favourite === false);
            expect(areAllNotFavourite).toBe(true);
        });
    });
});